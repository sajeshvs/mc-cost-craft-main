...
-- Update the resources table to support comprehensive job cost summary integration
...
ADD COLUMN IF NOT EXISTS final_rate NUMERIC GENERATED ALWAYS AS (
  CASE 
    ...
    ELSE rate * (1 - discount_percent / 100.0)
  END
) STORED,
ADD COLUMN IF NOT EXISTS linked_price_codes JSONB DEFAULT '[]'::jsonb;

-- Update existing computed columns to use final_rate instead of rate/bid_rate
ALTER TABLE public.resources 
DROP COLUMN IF EXISTS used_amount,
DROP COLUMN IF EXISTS wastage_amount,
DROP COLUMN IF EXISTS total_amount;

-- Add new computed columns using final_rate
ALTER TABLE public.resources 
